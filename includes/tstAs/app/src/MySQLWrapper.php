...
class MySQLWrapper
{
  ...
  public function store_login_var($p_login_key, $p_login_value)
  {

    if ($this->login_var_exists($p_login_key) === true)
    {
      $this->set_login_var($p_login_key, $p_login_value);
    }
    else
    {
      $this->create_login_var($p_login_key, $p_login_value);
    }

    return($this->c_arr_errors);
  }

  private function login_var_exists($p_login_key)
  {
    $login_var_exists = false;
    $m_query_string = $this->c_obj_sql_queries->check_login_var();

    $m_arr_query_parameters = [
      ':local_login_id' => login_id(),
      ':login_var_name' => $p_login_key
    ];
    ...
    {
      $login_var_exists = true;
    }
    return $login_var_exists;
  }

  private function create_login_var($p_login_key, $p_login_value)
  {
    $m_query_string = $this->c_obj_sql_queries->create_login_var();

    $m_arr_query_parameters = [
      ':local_login_id' => login_id(),
      ':login_var_name' => $p_login_key,
      ':login_var_value' => $p_login_value
    ];

    $this->safe_query($m_query_string, $m_arr_query_parameters);
  }

  private function set_login_var($p_login_key, $p_login_value)
  {
    $m_query_string = $this->c_obj_sql_queries->set_login_var();
